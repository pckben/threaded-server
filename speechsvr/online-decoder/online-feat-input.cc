...
OnlineCmvnInput::Compute(Matrix<BaseFloat> *output, uint32 *timeout) {
  ...
  cmvn_.ApplyCmvn2(in_matrix_, output);
  return more_data;
}
...
OnlineAudioInput::OnlineAudioInput(OnlineAudioSource* au_src, int32 frame_size)
	: source_(au_src), frame_size_(frame_size) {
  wave_.Resize(frame_size);
}

bool
OnlineAudioInput::Compute(Matrix<float> *output, uint32 *timeout) {
  int32 n = source_->Read(&wave_, timeout);

  if (timeout != 0 && *timeout == 0)
    KALDI_WARN << "InputAudioSource::Read() timeout expired!";
  else if (n != frame_size_)
    KALDI_VLOG(3) << frame_size_ << " samples were requested, but only "
                  << n << " were received!";

  // if all requested samples are obtained or a timeout was triggered,
  // then we assume there is more data in the audio source
  return ((n == frame_size_) || (timeout != 0 && *timeout == 0));
}


OnlineFeInput::OnlineFeInput(OnlineAudioSource *au_src, FeatureExtractor *fe,
                                   int32 frame_size, int32 frame_shift)
    : source_(au_src), extractor_(fe),
      frame_size_(frame_size), frame_shift_(frame_shift) {}

bool
OnlineFeInput::Compute(Matrix<BaseFloat> *output, uint32 *timeout) {
